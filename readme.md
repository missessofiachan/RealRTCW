...
* **Dynamic Sound Propagation & Geometric Wave-Tracing (Acoustic Diffraction)**: Replaces static linear occlusion with real-time wave diffraction (`S_AL_CalculateDiffractionHF`). Queries BSP area portals and evaluates 2-segment diffracted vectors to calculate the acoustic bending angle $\theta$. Audio bending around open doorways and hallway corners diffracts smoothly ($\text{gainHF} = \text{baseGain} \cdot \cos^2(\theta/2)$) while solid bunker walls remain properly occluded.
