...
* **Post-Process Anti-Aliasing Pipeline (`tr_postaa.c`)**: Integrates high-performance screen-space anti-aliasing algorithms (`r_postAA`: 0 = Off, 1 = FXAA, 2 = SMAA). Uses a full-screen FXAA 3.11 GLSL fragment shader pass and multi-pass morphological anti-aliasing executed via streamable vertex-array quad buffers to deliver clean edge smoothing without performance degradation or immediate-mode GL overhead.
...
* **Post-Process Anti-Aliasing UI Options (`r_postAA`)**: Adds an interactive **Post-Process AA** menu selector (`Off` / `FXAA` / `SMAA`) directly to the System Graphics menu (**Options -> Graphics**), controlling screen-space anti-aliasing passes rendered at full viewport resolution.
