...
#ifndef Q3_VM
    // Vectorized Bounding Box Union via SSE min/max
...
static int R_ComputeFogNum( md3Header_t *header, trRefEntity_t *ent ) {
    ...
    for ( i = 1 ; i < tr.world->numfogs ; i++ ) {
        ...
        // (sphere_min >= fog_max) OR (sphere_max <= fog_min)
        ...
        // Extract mask for the valid spatial axes (X, Y, Z bits corresponding to bits 0, 1, 2)
        int mask = _mm_movemask_ps( v_outside ) & 7;

        // If the mask is completely 0, the object overlaps the volume on all 3 axes simultaneously!
        if ( mask == 0 ) {
            return i;
        }
    }
